...
local lock        = "hyprlock"
local screenshot  = "hyprshot --freeze --mode=region --raw | satty --filename=- --config ~/.config/satty/config.toml"
...
hl.config({
  general = {
    ...
    col              = {
      active_border   = { colors = { "rgb(c6a0f6)", "rgb(91d7e3)" }, angle = 45 },
...
hl.bind(mainMod .. " + R", hl.dsp.exec_cmd('err=$(voxtype record toggle 2>&1) || notify-send "voxtype error" "$err"'))
hl.bind(mainMod .. " + T", hl.dsp.window.pseudo())

hl.bind("CTRL + Print",
  hl.dsp.exec_cmd(
    'hyprshot --freeze --mode=region --raw | tesseract stdin stdout -l por+eng | wl-copy'))
...
hl.bind(mainMod .. " + CTRL + Space", hl.dsp.window.move({ workspace = "special:magic" }))
...
-- Assign workspaces 1-5 to HDMI-A-1
...
-- Assign workspaces 6-10 to DP-1
...
hl.window_rule({
  name = "lan-mouse",
  match = { class = "de.feschber.LanMouse", },
  float = true
})
